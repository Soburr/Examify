...
<div style="display:flex; align-items:center; justify-content:space-between;
    ...
    <div>
        <div style="font-family:'Plus Jakarta Sans',sans-serif; font-weight:800;
                    font-size:20px; color:var(--blue-900);">📢 Notices</div>
        <div style="font-size:13px; color:var(--gray-400); margin-top:4px;">
            All school notices — you can post and delete any notice
        </div>
    </div>
    <button onclick="toggleForm('noticeForm')" id="toggleBtn" class="btn-primary">
        ＋ Post Notice
    </button>
</div>
...
<div id="noticeForm" style="display:none; margin-bottom:24px;">
    <div class="card">
        ...
        @if($errors->any())
            <div style="background:#fee2e2; border:1px solid #fca5a5; padding:12px 14px;
                        border-radius:8px; margin-bottom:16px;">
                <ul style="margin:0; padding-left:16px; color:#b91c1c; font-size:13px;">
        ...
        <form method="POST" action="{{ route('admin.notices.store') }}">
            ...
            {{-- Class selector --}}
            <div id="classSelector" style="display:none; margin-bottom:16px;">
            ...
            <div style="display:flex; gap:12px;">
                ...
                <button type="button" onclick="toggleForm('noticeForm')" class="btn-secondary">Cancel</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap; margin-bottom:16px;">
        ...
        @foreach($classes as $class)
            @if($notices->where('class_id', $class->id)->count() > 0)
                <button class="filter-btn" onclick="filterNotices('{{ $class->id }}', this)">
                    {{ $class->name }}
                </button>
            @endif
        @endforeach
                ...
                <div style="display:flex; align-items:flex-start; justify-content:space-between;
                    ...
                    <form method="POST" action="{{ route('admin.notices.destroy', $notice->id) }}"
                        ...
                        <button type="submit" class="btn-danger"
                                style="padding:7px 14px; font-size:12.5px; white-space:nowrap;">
                            🗑 Delete
                        </button>
                    </form>
                </div>
...
<style>
    .class-check-label {
        ...
        font-size: 13.5px; color: var(--gray-700); transition: all .2s;
        user-select: none; background: #fff;
    }
    .class-check-label:hover { border-color: var(--blue-400); background: var(--blue-50); }
    .filter-btn {
        padding: 6px 14px; border-radius: 20px; border: 1.5px solid var(--gray-200);
        background: #fff; color: var(--gray-500); font-size: 12.5px; font-weight: 600;
...
<script>
    function toggleForm(id) {
        ...
        if (btn) btn.textContent = open ? '✕ Close' : '＋ Post Notice';
    ...
    function toggleClassSelect() {
        ...
        document.getElementById('classSelector').style.display = val === 'class' ? 'block' : 'none';
    }
    ...
    function filterNotices(type, btn) {
        ...
        document.querySelectorAll('.notice-item').forEach(item => {
            ...
            else if (type === 'schoolwide') item.style.display = item.dataset.type === 'schoolwide' ? 'block' : 'none';
            else item.style.display = item.dataset.class == type ? 'block' : 'none';
        });
    }
